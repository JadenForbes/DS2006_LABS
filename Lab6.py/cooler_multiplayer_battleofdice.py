...
while all(wins < win_goal for _, wins in players):
    ...
    for player in players:
        ...
        roll2 = dice.roll_d8()
        total = roll1 + roll2
        round_results.append([player_num, total])

        print(f"Player {player_num} rolled: d6={roll1}, d8={roll2} → Total = {total}")

    input("Press ENTER to see who won the round...")

    # Find highest roll in this round
    max_total = max(total for _, total in round_results)
    winners = [player_num for player_num, total in round_results if total == max_total]
